...
from marker_detector import ArucoMarkerDetector
from person_detector import build_person_detector
from ptz_controller import PTZCommand, PTZSimulator
from target_selector import TargetSelector, point_in_bbox
from target_state import ActiveTarget, DemoConfig, TrackingState
...
def main() -> int:
    ...
    while state is not TrackingState.ENDED:
        ...
        if target is not None and state is TrackingState.ACTIVE:
        ...
        if args.log_csv is not None:
            ...
            write_csv_row(csv_writer, state, target, last_command)
...
def open_csv_logger(output_path: str):
    ...
    writer.writerow(["timestamp", "state", "target_id", "marker_id", "bbox", "ptz_command", "error_x", "error_y"])
    return csv_file, writer


def write_csv_row(writer, state: TrackingState, target: Optional[ActiveTarget], command: PTZCommand) -> None:
    bbox = ""
    target_id = ""
    marker_id = ""
...
def draw_debug(
    ...
    overlay = (
        ...
        f"marker_only={marker_only} gesture={gesture_enabled}"
    )
